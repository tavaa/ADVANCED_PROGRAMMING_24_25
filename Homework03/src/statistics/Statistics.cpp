...
/*
* Public constructor of Statistics, recalls an instance of CSVParser to parse the data
*/
Statistics::Statistics(const std::string& filePath, char delimiter, bool hasHeader) : csvParser(filePath, delimiter, hasHeader) {
    data = csvParser.parse();
}


/*
* Getter for headers within the dataset
* @return parsed_dataset_headers
*/
std::optional<std::vector<std::string>> Statistics::getHeaders() const {
    return csvParser.getHeaders();
}

/*
* Getter for Column Values of a specific column of the dataset
* @return vector of cells containing the values of the column
* @throws run_time_error
*/
std::optional<std::vector<Cell>> Statistics::getColumn(const std::string& columnName) const {
    try {
        return csvParser[columnName];
    } catch (const std::runtime_error& e) {
        throw std::runtime_error("Column '" + columnName + "' does not exist in the dataset.");
    }
}
...
void Statistics::drawHistogram(const std::vector<Cell> &columnValues, std::ostream& out) {
    ...
    for (const auto& pair : freqMap) {
        ...
        // Scale the number of asterisks to the frequency
        int barLength = static_cast<int>(50.0 * pair.second / maxFrequency);  // Scale bar to max 50 chars
